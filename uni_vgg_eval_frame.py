...
from models.modeling_resnet import UniVGGNet
from param import *
from param import get_prefix
...
for _, row in tqdm(data.iterrows(), total=data.shape[0]):
    ...
    dataset_split = capture_dir if prefix == '' else prefix.split('_')[0]
    for frame_id, pred_label in zip(range(start_frame, end_frame), class_idx):
        label_pair.append(
            (dataset_split, capture_id, frame_id, pred_label, label)
        )

    duration = time.time() - start_time
    response_time.append((dataset_split, capture_id, duration))
    # print(capture_id, class_names[class_idx], label)

out_labels = pd.DataFrame(label_pair, columns=[
    'Dataset', 'Capture_ID', 'Frame_ID', 'Prediction', 'Ground_Truth'
])
out_labels.to_csv(f'{exp_name}/{exp_name}_frame_labels_out.csv', index=False)

response_time = pd.DataFrame(response_time, columns=['Dataset', 'Capture_ID', 'Duration'])
response_time.to_csv(f'{exp_name}/{exp_name}_response_time.csv', index=False)

label_pair = np.array(label_pair)
print(label_pair.shape)

y_pred, y_true = label_pair[:, 3], label_pair[:, 4]
...
results.to_csv(f'{exp_name}/{exp_name}_frame_results.csv')
...
plt.tight_layout()
plt.savefig(f'{exp_name}/{exp_name}_frame_cm.png', dpi=300, bbox_inches='tight')
