...
def projection(cam_data_dir, color_data, depth_data):
    ...
    for frame_idx in range(frame_count):
        point_cloud = pc_frames[frame_idx]
        point_cloud = point_cloud[sort_idx]
        xs, zs = point_cloud[:, 0], point_cloud[:, 2]
        xs, zs = xs.astype(np.int16), zs.astype(np.int16)
        cs = point_cloud[:, 3:6] / 255
        valid_pts = (np.abs(xs) < 40) & (zs < 80)
        xs, zs, cs = xs[valid_pts] + 40, zs[valid_pts], cs[valid_pts]
        rgbd_data[frame_idx, zs, xs] = cs
...
response_time_out.to_csv(f'{dataset_type}_response_time_out.csv', index=False)
