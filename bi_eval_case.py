...
from param import image_transform, get_prefix, get_class_names
...
for _, row in tqdm(data.iterrows(), total=data.shape[0]):
    ...
    label_pair.append(
        (dataset_split, capture_id, class_names[class_idx], label)
    )
    # print(capture_id, class_names[class_idx], label)

out_labels = pd.DataFrame(label_pair, columns=['Dataset', 'Capture ID', 'Prediction', 'Ground Truth'])
...
y_pred, y_true = label_pair[:, 2], label_pair[:, 3]
