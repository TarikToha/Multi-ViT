...
from models.modeling import BiVisionTransformer
from param import *
...
data_dir = f'bi_{dataset_type}_{sys.argv[3]}'
ckpt_path = f'{exp_name}/{exp_name}_checkpoint.bin'

class_names = get_class_names(dataset_type)
num_class = len(class_names)

model = BiVisionTransformer(get_b16_config(), num_classes=num_class)
...
for _, row in tqdm(data.iterrows(), total=data.shape[0]):
    ...
    for frame_id in range(start_frame, end_frame):
        ...
        mode2_img = f'{mode2_in_path}{prefix}{capture_id}_{frame_id}.jpg'
    ...
    for frame_id, pred_label in zip(range(start_frame, end_frame), class_idx):
        label_pair.append((dataset_split, capture_id, frame_id, pred_label, label))
